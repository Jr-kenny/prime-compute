...
// `Provider.specs` is `Record<string, unknown>` (a jsonb column with no fixed shape), which is
// genuinely JSON-serializable at runtime but TanStack Start's static serializability check can't
// prove that for an `unknown`-valued index signature. `strict: { output: false }` skips that
// static check for the three functions below that return a `Provider`; the wire format is still
