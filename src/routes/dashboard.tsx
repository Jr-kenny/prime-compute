        ...
        <Tabs defaultValue="active" className="mt-8">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs uppercase tracking-wider text-muted-foreground text-left">
                  <th className="py-2">Job</th><th>Provider</th><th>Duration</th><th>Cost</th><th>Status</th><th>Date</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {historicalJobs.map((j) => (
                  <tr key={j.id}>
                    ...
                    <td className="text-muted-foreground text-xs">{new Date(j.startedAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <TabsContent value="billing" className="mt-6 grid gap-6 lg:grid-cols-3">
            <div className="glass-card p-6">
              ...
              <Button className="mt-5 w-full bg-primary text-primary-foreground"><Plus className="h-4 w-4" /> Add funds</Button>
            </div>
              ...
              <div className="h-56">
                <ResponsiveContainer>
                  <AreaChart data={spending30d}>
                    ...
                    <Area type="monotone" dataKey="spent" stroke="var(--color-glow)" fill="url(#sg)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
          ...
          <TabsContent value="settings" className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="glass-card p-6 space-y-4">
              ...
              <div className="flex items-center gap-2">
                <Input readOnly value="pc_live_••••••••••••••sk29x" className="font-mono bg-card border-border" />
                <Button variant="ghost" size="icon" className="border border-border"><Copy className="h-4 w-4" /></Button>
              </div>
  ...
  return (
    <div className="glass-card glow-hover p-5">
      ...
      <div className="mt-4 flex items-end justify-between">
        <div>
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Streaming spend</div>
          <StreamingTicker
        ...
        <div className="text-right">
          ...
          <div className="text-sm text-foreground"><ElapsedTimer startedAt={job.startedAt} paused={paused} /></div>
        </div>
      </div>
      ...
      <div className="mt-5 flex gap-2">
        <Button variant="ghost" className="flex-1 border border-border" onClick={() => setPaused((v) => !v)}>
          <Pause className="h-4 w-4" /> {paused ? "Resume" : "Pause"}
        </Button>
        <Button variant="ghost" className="flex-1 border border-destructive/30 text-destructive hover:bg-destructive/10">
  ...
  return (
    <div>
      ...
      <div className="mt-1 h-1.5 rounded-full bg-border overflow-hidden">
        <div className="h-full bg-gradient-to-r from-accent to-glow" style={{ width: `${value}%` }} />
      </div>
    </div>
  );
}
