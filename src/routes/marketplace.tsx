...
export const Route = createFileRoute("/marketplace")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse live providers offering GPUs, CPUs, and storage with per-second pricing." },
      { property: "og:title", content: "Compute Marketplace — Prime Compute" },
      { property: "og:description", content: "Browse live providers offering GPUs, CPUs, and storage with per-second pricing." },
    ],
  }),
...
function MarketplacePage() {
  ...
  const filtered = useMemo(() => {
    return providers.filter((p) => {
      ...
      if (!types.includes(p.resourceType as ResourceType) && p.resourceType !== "Full Server") return false;
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10">
        <header className="flex flex-wrap items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{filtered.length} providers match your filters</p>
          </div>
            ...
            <div className="lg:hidden mb-4">
              <Sheet>
                <SheetTrigger asChild>
                  <Button variant="ghost" className="border border-border w-full"><FilterIcon className="h-4 w-4" /> Filters</Button>
                </SheetTrigger>
...
function FiltersPanel({
  types, toggleType, minScore, setMinScore, maxPrice, setMaxPrice, availableOnly, setAvailableOnly,
}: {
  types: ResourceType[];
  ...
  return (
    <div className="space-y-6 glass-card p-5">
      <div>
        <Label className="text-xs uppercase tracking-wider text-muted-foreground">Resource type</Label>
      ...
      <div>
        <div className="flex items-center justify-between">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">Min compute score</Label>
          <span className="text-xs text-foreground">{minScore}</span>
        </div>
        <Slider className="mt-3" value={[minScore]} min={0} max={100} step={1} onValueChange={(v) => setMinScore(v[0])} />
      </div>
      <div>
        <div className="flex items-center justify-between">
          <Label className="text-xs uppercase tracking-wider text-muted-foreground">Max $/sec</Label>
          <span className="text-xs text-foreground">${maxPrice.toFixed(7)}</span>
        </div>
        <Slider className="mt-3" value={[maxPrice]} min={0.000001} max={0.00003} step={0.0000005} onValueChange={(v) => setMaxPrice(v[0])} />
      </div>
      <div className="flex items-center justify-between">
...
function DeploySheet({ provider, onClose }: { provider: Provider | null; onClose: () => void }) {
  ...
  return (
    <Sheet open={!!provider} onOpenChange={(o) => { if (!o) { onClose(); setDone(false); setName(""); } }}>
      <SheetContent className="bg-surface border-border">
        ...
        {provider && !done && (
          <div className="mt-6 space-y-5">
            <div>
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. llama-fine-tune" className="mt-2 bg-card border-border" />
            </div>
            ...
            <div>
              ...
              <Slider className="mt-3" value={[duration]} min={1} max={240} step={1} onValueChange={(v) => setDuration(v[0])} />
            </div>
            <div className="glass-card p-4">
              ...
              <div className="mt-1 text-[11px] text-muted-foreground">at ${provider.pricePerSecond.toFixed(7)}/s · streaming, refundable on cancel</div>
            </div>
            <SheetFooter>
              <Button onClick={submit} disabled={submitting || !name} className="w-full bg-primary text-primary-foreground">
        ...
        {done && (
          <div className="mt-12 text-center">
            <div className="mx-auto h-12 w-12 rounded-full bg-success/15 ring-1 ring-success/40 flex items-center justify-center text-success">✓</div>
            <h3 className="mt-4 text-lg font-semibold">Job submitted</h3>
            <p className="mt-1 text-sm text-muted-foreground">The broker is opening a payment stream now.</p>
            <Button onClick={() => { onClose(); setDone(false); setName(""); }} variant="ghost" className="mt-6 border border-border">Close</Button>
          </div>
        )}
      </SheetContent>
    </Sheet>
